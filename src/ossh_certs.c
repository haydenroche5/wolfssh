...
static WOLFSSH_OSSH_PRINCIPAL* OsshCertPrincipalNew(void* heap)
{
...
static void OsshCertPrincipalListFree(WOLFSSH_OSSH_PRINCIPAL* principal,
                                      void* heap)
{
    WOLFSSH_OSSH_PRINCIPAL* tmp;

    (void)heap;

    while (principal != NULL) {
        tmp = principal->next;
        WFREE(principal, heap, DYNTYPE_OSSH_PRINCIPAL);
        principal = tmp;
    }
}

static int GetValidPrincipals(byte* in, word32 inSz, word32* idx,
                              WOLFSSH_OSSH_PRINCIPAL** out, void* heap)
{
    int ret = WS_SUCCESS;
    word32 listSz;
    byte* name = NULL;
    word32 nameSz;
    WOLFSSH_OSSH_PRINCIPAL* current = NULL;
    WOLFSSH_OSSH_PRINCIPAL* next;
    WOLFSSH_OSSH_PRINCIPAL* first = NULL;
    ...
    while (ret == WS_SUCCESS && listSz > 0) {
        ...
        if (ret == WS_SUCCESS) {
            if (nameSz > MAX_OSSH_PRINCIPAL_MAX_NAME_SZ) {
                ...
                next = OsshCertPrincipalNew(heap);
                if (next == NULL) {
                    ret = WS_MEMORY_E;
                }
                else {
                    WMEMCPY(next->name, name, nameSz);
                    next->nameSz = nameSz;
                    if (current != NULL) {
                        current->next = next;
                    }
    ...
    if (ret == WS_SUCCESS) {
        *out = first;
        ...
        if (first != NULL) {
            OsshCertPrincipalListFree(first, heap);
        }
    }
...
void OsshCertFree(WOLFSSH_OSSH_CERT* cert, void* heap)
{
    (void)heap;

    if (cert != NULL) {
        if (cert->principals != NULL) {
            OsshCertPrincipalListFree(cert->principals, heap);
        }
        if (cert->pubKey != NULL) {
            switch (cert->type) {
            ...
            WFREE(cert->pubKey, heap, DYNTYPE_PUBKEY);
        }

        WFREE(cert, heap, DYNTYPE_OSSH_CERT);
    }
}
                  ...
                  void* heap)
{
    ...
    WOLFSSH_OSSH_PRINCIPAL* principals = NULL;
